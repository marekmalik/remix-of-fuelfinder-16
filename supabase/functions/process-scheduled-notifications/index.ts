...
async function generateVapidAuth(
  ...
  const pkcs8Key = new Uint8Array(
    pkcs8Header.length + 32 + pkcs8Footer.length + publicKeyBytes.length,
  );
  ...
  try {
    ...
    const dueSchedules = schedules.filter((schedule) => {
      ...
      if (schedule.schedule_type === "interval") {
        ...
        if (isDue) {
          console.log(
            `Interval schedule ${schedule.id} is due: every ${schedule.interval_minutes}min from ${schedule.from_time} to ${schedule.until_time} in ${timezone}, current time: ${localTime}`,
          );
        }
        return isDue;
      }
      ...
      if (timeMatches && dayMatches) {
        console.log(
          `Daily schedule ${schedule.id} matches: ${scheduleTime} in ${timezone}`,
        );
      }
    ...
    const results = await Promise.allSettled(
      dueSchedules.map(async (schedule) => {
        ...
        if (subError) {
          console.error(
            "Error fetching subscriptions for user:",
            schedule.user_id,
          );
        ...
        console.log(
          `Sent ${sentCount} notifications for user: ${schedule.user_id}`,
        );
    ...
    const totalSent = results
      .filter(
        (r): r is PromiseFulfilledResult<{ userId: string; sent: number }> =>
          r.status === "fulfilled",
      )
