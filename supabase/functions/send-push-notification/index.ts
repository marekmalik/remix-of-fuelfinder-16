...
async function sendWebPush(
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      `Push service returned ${response.status}: ${errorBody}`,
    );
...
serve(async (req) => {
  ...
  try {
    ...
    const results = await Promise.allSettled(
      subscriptions.map(async (sub) => {
        try {
          ...
          console.log(
            "Push notification sent successfully to subscription:",
            sub.id,
          );
    ...
    console.log(
      `Successfully sent ${successCount} of ${subscriptions.length} notifications`,
    );
